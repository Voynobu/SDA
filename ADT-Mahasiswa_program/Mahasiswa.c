...
void Isi_Node (address *p , infotype nilai, char *name){
...
void Del_Awal(address *p, infotype *X){
    ...
    *p = temp->next; 
	free(temp->name);         // Pindahkan head ke node berikutnya
...
void CopytoList70(address p, address *First2) {
    ...
    while (curr != NULL) {
        if (curr->info > 70) {
            ...
            newNode->name = (char *)malloc((strlen(curr->name) + 1) * sizeof(char));
